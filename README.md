...
- ### Fichier .csv:
  Dans un fichier .csv, sur chaque ligne vous devrez inscrire le modèle, la marque, le type, la consommation puis le prix des modèles affichés par le concessionnaire voulu. Un maximum de 19 marques différentes peuvent y être inscrites. Bien important de précéder chaque information par un point-virgule ( ; ). Si vous créez votre .csv dans excel, assurez-vous de laisser la colonne "A" vide. Aucune autre ponctuation ne devrait être présente dans le fichier. Inscrire les prix de la même façon que l'exemple fournis plus bas.
